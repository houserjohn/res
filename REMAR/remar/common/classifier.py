...
class Rating_Classifier(nn.Module):
    """
    The Encoder takes an input text (and rationale z) and computes p(y|x,z)
    ...
    def __init__(self,
        ...
        super(Rating_Classifier, self).__init__()
    ...
    def forward(self, src):
        ...
        # predict sentiment from final state(s)
        y = self.output_layer(src)
     
        return y 



class Polarity_Classifier(nn.Module):
    """
    The Encoder takes an input text (and rationale z) and computes p(y|x,z)
    ...
    def __init__(self,
        ...
        super(Polarity_Classifier, self).__init__()
    ...
    def forward(self, y, src):

        y = torch.cat((y,src), dim = 1)

        # predict sentiment from final state(s)
        final = self.output_layer(y)

        return final


class Testing_Classifier(nn.Module):
    """
    The Encoder takes an input text (and rationale z) and computes p(y|x,z)
    ...
    def __init__(self,
        ...
        super(Testing_Classifier, self).__init__()
        self.aspects = aspects
        self.output_layer = nn.Sequential(
            nn.Dropout(p=dropout),
            nn.Linear(hidden_size*2*(self.aspects), hidden_size),
            nn.Linear(hidden_size, 1),
            # nn.Sigmoid() 
        )
