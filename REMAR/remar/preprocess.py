...
def parse_amazon(args):
    ...
    """
    assert hasattr(args, "amazon_subset"), "Please set `amazon_subset` for Amazon dataset."
    ...
    train, test = train_test_split(df, test_size=test_split_ratio)
    train.to_csv('sTrainData.csv')
    test.to_csv('sTestData.csv')
...
def parse_goodreads(args):
    ...
    json_data = []
    with open('goodreads.json', 'r') as handle: # replace "goodreads.json" with the path of your file
            for line in handle:
                json_data.append(json.loads(line))
        
    data_raw = pd.DataFrame(json_data)
    data = data_raw.drop(['review_id', 'date_added', 'date_updated', 'read_at', 'started_at', 'n_votes', 'n_comments'], axis=1)
    ...
    G = nx.Graph()
    G.add_edges_from(zip(data['user_id'], data['book_id'])) #uids for authors, bids for books
    ...
    kcore_df['review_text'] = pd.DataFrame(data.review_text.apply(stemFunction))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--min_cat_num", type=int, default=2,
        help="Minimum number of category labels in the filter set.")
    parser.add_argument("--amazon_subset", type=str, help="Subset name of Amazon dataset")
    parser.add_argument("--test_split_ratio", type=int, default=0.05,
